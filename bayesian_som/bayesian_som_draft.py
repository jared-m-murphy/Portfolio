...
def running_covariance(label_list):
    ...
    for label in label_list:
        ...
        #image_list will contain all images to print
        images_for_print=[]
        #make pixel list for each image and concat to running_pixe_list
        for image in image_list[1:]:
            pixel_list=pixel_list_creator([image])
            running_pixel_list = np.concatenate((running_pixel_list,pixel_list), axis=1)
            measuring_stick=len(running_pixel_list[0])
            if (measuring_stick % 100 == 0.0):
                mu=mean_vector_simple(running_pixel_list)
                covariance=cov_matrix(running_pixel_list)
                sample_image=multivariate_sample(mu,covariance)
                images_for_print.append(sample_image)
        label_image_lists.append(images_for_print[:50])
    for i in range(len(label_image_lists)):
        x=label_image_lists[i]
        num_row = 10
        num_col = 5 
        fig, axes = plt.subplots(num_row, num_col, figsize=(1.5*num_col,2*num_row))
        for j in range(len(x)):
            ...
            ax.set_title('Iteration: {}'.format(str((j+1)*100)))
        plt.tight_layout()
        plt.savefig("running_covariance_simple_" + str(i))
        plt.show()  
    

running_covariance(label_list)
